...
local adg_rules_local=luci.sys.exec("cat /usr/share/koolproxy/data/rules/adg.txt | sed -n '3p'|awk '{print $3,$4}'")
local adg_nu_local=luci.sys.exec("grep -v '^!' /usr/share/koolproxy/data/rules/adg.txt | wc -l")
local antiad_rules_local=luci.sys.exec("cat /usr/share/koolproxy/data/rules/antiad.txt | sed -n '4p'|awk '{print $3,$4}'")
local antiad_nu_local=luci.sys.exec("grep -v '^!' /usr/share/koolproxy/data/rules/antiad.txt | wc -l")
...
e.optional = false
e.rmempty = false
e:value("adg.txt", translate("ADG-ilxp规则"))
e:value("antiad.txt", translate("AN-ilxp规则"))
